...
# First plot (Number of guesses)
p1 <- ggplot(data=df_sub, aes(x=num_particles, y=guess_number, group=num_particles)) +
...
# Second plot (EIG quantile)
p2 <- ggplot(data=subset(df, (df$solved == 0) | (df$true_entropy_prev > 0)), aes(x=num_particles, y=EIG_quantile, group=num_particles)) +
  geom_boxplot(outliers=FALSE) +
  geom_hline(yintercept=0.5, linetype="dotted") +
...
ggplot(subset(df, (df$solved == 0) | (df$true_entropy_prev > 0)), aes(x = round(EIG_gt_eq, 1), group = num_particles)) +
  geom_bar(aes(y = ..prop..), stat = "count") +
  paper_theme +
  facet_wrap(~num_particles, nrow = 1) +
  ylab("Proportion") + xlab("EIG quantile (gte)") +
  scale_x_continuous(breaks=c(0,0.5, 1)) +
  theme(axis.text.x = element_text(angle = 0), strip.text = element_text(size = 12))
